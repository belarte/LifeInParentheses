...
(defn- make-face-direction [direction expression]
  (let [d (-> expression :alu/output :alu/direction)]
    (if (= d direction) expression (layout/flip-x expression))))
...
(defn not-bit
  ...
  ;{:pre [(s/valid? :alu/expression expression) (layout/within-bounds? expression)]
  ; :post [(s/valid? :alu/expression %) (layout/within-bounds? %)]]
  (let [e (make-face-direction :bottom-right expression)
...
(defn and-bit
  ...
  ; ;:post [(s/valid? :alu/expression %) (layout/within-bounds? %)]}
  (let [left-e      (make-face-direction :bottom-right left)
        right-e     (make-face-direction :bottom-right right)
